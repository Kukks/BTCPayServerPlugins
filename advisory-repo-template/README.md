...
and a rotation that would drop the trust store below the quorum threshold — not just down to
zero — is refused.
...
`.nojekyll` is committed but is a no-op for the Actions-based deployment this template's
workflow uses (`upload-pages-artifact` + `deploy-pages` never run Jekyll, and the artifact
upload excludes dotfiles by default regardless). It matters only if this repo's Pages source is
ever switched to legacy branch-based deployment, where Jekyll does run and does exclude
dot-prefixed paths — kept as cheap insurance against that case.
